...
- 🔴 **`docs/` は 1 本ではない。**触る領域に応じて [monitoring.md](monitoring.md) / [sources.md](sources.md) / [daemon.md](daemon.md) も開く。⚠⚠ **自動ロードされるのは `CLAUDE.md` と `MEMORY.md` だけ**なので、**「書いていない」と判断する前に `grep -rn <語> docs/` でまとめて引く**
